// src/components/compliance/VATDetail.tsx
'use client';
...
interface ComputationBreakdown {
  // Current Sales and Output Tax
  vatableSales: number;
  salesToGovernment: number;
  zeroRatedSales: number;
  exemptSales: number;
  // Allowable Input Tax
  inputTaxCarriedOver: number;
  // Current Purchases and Input Tax
  capitalGoodsNotExceeding1M: number;
  capitalGoodsExceeding1M: number;
  domesticPurchasesGoods: number;
  importationGoods: number;
  domesticPurchasesServices: number;
  purchasesNotQualified: number;
}

interface QuarterRecord {
  id: string;             // e.g. "vat-2026-q1"
  year: number;
  quarterIdx: number;     // 0-3
  quarterLabel: string;   // e.g. "1st Quarter"
  months: MonthRow[];
  ...
  computation: ComputationBreakdown;
}
...
interface MonthRow {
  ...
  outputVAT: number;
  inputVAT: number;
  vat12: number;       // outputVAT - inputVAT
  salesFinalized: boolean;
  outputRecords: OutputVATRecord[];
}
...
function buildMockOutputRecords(year: number, monthIdx: number): OutputVATRecord[] {
  const suppliers = [
  ...
  const accounts = ['Service Revenue', 'Product Sales', 'Consulting Fees', 'Rental Income', 'Commission Income'];
  const types = ['Sales Invoice', 'Service Invoice', 'Invoice'];
  const result: OutputVATRecord[] = [];
  for (let i = 0; i < 5; i++) {
    const gross = Math.round((12000 + i * 9500 + monthIdx * 1200) * 100) / 100;
    const net = Math.round((gross / 1.12) * 100) / 100;
    const vat = Math.round((net * 0.12) * 100) / 100;
    result.push({
      id: `ovr-${year}-${monthIdx}-${i}`,
      ...
      supplierVat: suppliers[i % suppliers.length]!.vat,
...
function buildDefaultComputation(outputVAT: number, inputVAT: number): ComputationBreakdown {
  return {
    vatableSales: Math.round(outputVAT / 1.12 * 100) / 100,
    salesToGovernment: 0,
    zeroRatedSales: 0,
    exemptSales: 0,
    inputTaxCarriedOver: 0,
    capitalGoodsNotExceeding1M: Math.round((inputVAT * 0.3) * 100) / 100,
    capitalGoodsExceeding1M: 0,
    domesticPurchasesGoods: Math.round((inputVAT * 0.4) * 100) / 100,
    importationGoods: 0,
    domesticPurchasesServices: Math.round((inputVAT * 0.3) * 100) / 100,
    purchasesNotQualified: 0,
  };
}

// Per-month overrides for 2026 demo data
// Key: monthIdx → { inputMultiplier, salesFinalized }
// Jan Q1: large capital goods purchase makes Q1 total negative → triggers carry-over in Q2
// Mar Q1: sales book not yet finalized (pending example)
const MONTH_OVERRIDES_2026: Record<number, { inputMultiplier: number; salesFinalized: boolean }> = {
  0: { inputMultiplier: 3.5,  salesFinalized: true  }, // Jan — capital purchase inflates input
  1: { inputMultiplier: 0.55, salesFinalized: true  }, // Feb
  2: { inputMultiplier: 0.4,  salesFinalized: false }, // Mar — pending
  3: { inputMultiplier: 0.6,  salesFinalized: false }, // Apr — current month
};
  ...
  return QUARTER_MONTHS.map((monthIndices, qi) => {
    const months: MonthRow[] = monthIndices
      ...
      .map(mi => {
        ...
        const outputRecords = hasData ? buildMockOutputRecords(year, mi) : [];
        const outputVAT = outputRecords.reduce((s, r) => s + r.vat12, 0);
        // Use month-specific override multiplier for 2026, else default 0.6
        const override = year === today.getFullYear() ? MONTH_OVERRIDES_2026[mi] : undefined;
        const inputMultiplier = override?.inputMultiplier ?? 0.6;
        const inputVAT = hasData ? Math.round(outputVAT * inputMultiplier * 100) / 100 : 0;
        ...
        return {
          ...
          outputVAT,
          inputVAT,
          vat12: Math.round((outputVAT - inputVAT) * 100) / 100,
          salesFinalized,
          outputRecords,
        };
      });

    const totalOutput = months.reduce((s, m) => s + m.outputVAT, 0);
    const totalInput  = months.reduce((s, m) => s + m.inputVAT, 0);
    const vatPayable  = Math.round((totalOutput - totalInput) * 100) / 100;
    ...
    const docReqs = DOC_REQUIREMENT_LABELS.map((label, di) => ({
      id: `vat-doc-${year}-q${qi}-${di}`,
      label,
      url: isPaid ? `https://drive.google.com/file/vat-q${qi + 1}-${di}` : '',
    }));

    const processSteps = PROCESS_STEP_DEFS.map((def, si) => ({
      id: `vat-step-${year}-q${qi}-${si}`,
      role: def.role,
      label: def.label,
      acknowledgedBy: isPaid ? ['Juan Dela Cruz', 'Maria Santos', 'Pedro Reyes', 'Ana Gonzales', 'CEO Office'][si]! : '',
      acknowledgedAt: isPaid ? `${year}-${String(monthIndices[2]! + 1).padStart(2, '0')}-20` : '',
    }));
    ...
    return {
      ...
      months,
      paymentStatus: isPaid ? 'Paid' : vatPayable <= 0 ? 'Unpaid' : 'Unpaid',
      isAmended: false,
      taxWithheld: Math.abs(vatPayable),
      paidByClient: isPaid ? Math.abs(vatPayable) : 0,
      zeroFiling: vatPayable <= 0,
      taxInfo: {
        tin: '123-456-789-000',
        rdo: '082',
      ...
      processSteps,
      computation: buildDefaultComputation(totalOutput, totalInput),
    };
  });
}
...
export function VATDetail({ client, year, onYearChange }: VATDetailProps): React.ReactNode {
  // ── Quarter records ────────────────────────────────────────────────────────
  ...
  const [outputModalMonth, setOutputModalMonth] = useState<MonthRow | null>(null);
  ...
  // ── Computation edit state (inside VAT modal) ──────────────────────────────
  const [editComputation, setEditComputation] = useState<ComputationBreakdown | null>(null);
  ...
  function openOutputModal(month: MonthRow) {
    setOutputModalMonth(month);
    setOutputModalOpen(true);
  }

  function openVATModal(q: QuarterRecord) {
    setSelectedQuarter(q);
    setEditComputation({ ...q.computation });
  ...
  function saveComputation(quarterId: string) {
    if (!editComputation) return;
    setQuarters(prev => prev.map(q => {
      if (q.id !== quarterId) return q;
      const updated = { ...q, computation: { ...editComputation } };
      if (selectedQuarter?.id === quarterId) setSelectedQuarter(updated);
      return updated;
    }));
  }
  ...
  // ── Grand totals ───────────────────────────────────────────────────────────
  const grandOutputVAT = quarters.reduce((s, q) => s + q.months.reduce((ms, m) => ms + m.outputVAT, 0), 0);
  const grandInputVAT  = quarters.reduce((s, q) => s + q.months.reduce((ms, m) => ms + m.inputVAT,  0), 0);
  const grandVAT12     = quarters.reduce((s, q) => s + q.months.reduce((ms, m) => ms + m.vat12,     0), 0);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl font-black text-slate-900 tracking-tight">Value-Added Tax</h1>
            </div>
          </div>
          ...
          <table className="w-full text-sm border-collapse" style={{ minWidth: '900px' }}>
            <thead>
              <tr className="border-b border-slate-200 bg-white">
                ...
                <th className="text-right px-5 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500">Output VAT</th>
                <th className="text-right px-5 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500">Input VAT</th>
                <th className="text-right px-5 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500">VAT (12%)</th>
            ...
            <tbody>
              {quarters.map((q, qi) => {
                const qOutputVAT = q.months.reduce((s, m) => s + m.outputVAT, 0);
                const qInputVAT  = q.months.reduce((s, m) => s + m.inputVAT,  0);
                const qVAT12     = q.months.reduce((s, m) => s + m.vat12,     0);
                // carry over from previous quarter
                const prevQVAT = qi > 0
                  ? quarters[qi - 1]!.months.reduce((s, m) => s + m.vat12, 0)
                  : 0;
                const carryOver = prevQVAT < 0 ? Math.abs(prevQVAT) : 0;
                const netPayable = qVAT12 - carryOver;
                  ...
                  <React.Fragment key={q.id}>
                    ...
                    {q.months.map((month, mi) => (
                      ...
                      >
                        ...
                        {/* Output VAT — clickable */}
                        <td className="px-5 py-3.5 text-right">
                          {month.outputVAT > 0 ? (
                            <button
                              onClick={() => openOutputModal(month)}
                        ...
                        <td className="px-5 py-3.5 text-right font-mono text-sm text-slate-700">
                          {month.inputVAT > 0 ? fmtPHP(month.inputVAT) : <span className="text-slate-300">—</span>}
                        </td>
                        <td className={`px-5 py-3.5 text-right font-mono text-sm font-semibold ${month.vat12 < 0 ? 'text-red-600' : 'text-slate-800'}`}>
                          {month.outputVAT > 0 || month.inputVAT > 0 ? fmtPHP(month.vat12) : <span className="text-slate-300">—</span>}
                        </td>
                        <td className="px-5 py-3.5 text-center">
                          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wide ${
                            ...
                            {month.salesFinalized ? 'Finalized' : month.outputVAT > 0 ? 'Pending' : '—'}
                          </span>
                        </td>
                    ...
                    <tr className="border-b border-slate-200 bg-slate-100">
                      ...
                      <td className={`px-5 py-2.5 text-right font-mono text-xs font-black ${qVAT12 < 0 ? 'text-red-600' : 'text-slate-800'}`}>
                        {fmtPHP(qVAT12)}
                      </td>
                      <td colSpan={3} />
                    </tr>

                    {/* Carry-over row (only if previous quarter was negative) */}
                    {carryOver > 0 && (
                      <tr className="border-b border-slate-200 bg-slate-50">
                        <td className="px-5 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-500 italic">
                          Input Tax from Previous Quarter
                        </td>
                        <td />
                        <td className="px-5 py-2 text-right font-mono text-xs font-semibold text-slate-700">
                          {fmtPHP(carryOver)}
                        </td>
                        <td />
                        <td colSpan={3} />
                    ...
                    <tr className="border-b-2 border-slate-300 bg-slate-800">
                      ...
                      <td colSpan={2} />
                      <td className={`px-5 py-3 text-right font-mono text-sm font-black ${netPayable < 0 ? 'text-red-400' : 'text-white'}`}>
                        {fmtPHP(netPayable)}
                      </td>
                      <td colSpan={3} />
                    </tr>

                    {/* Spacer row between quarters */}
              ...
              <tr className="bg-slate-900 border-t-2 border-slate-400">
                ...
                <td className="px-5 py-3.5 text-right font-mono text-sm font-black text-white">
                  {fmtPHP(grandOutputVAT)}
                </td>
                <td className="px-5 py-3.5 text-right font-mono text-sm font-black text-white">
                  {fmtPHP(grandInputVAT)}
                </td>
                <td className={`px-5 py-3.5 text-right font-mono text-sm font-black ${grandVAT12 < 0 ? 'text-red-400' : 'text-white'}`}>
                  {fmtPHP(grandVAT12)}
                </td>
                <td colSpan={3} />
              </tr>
      ...
      {/* ══════════════════════════════════════════════════════════════
          MODAL 1 — Output VAT Receipt Detail
      ══════════════════════════════════════════════════════════════ */}
      <Modal
          ...
          <div className="overflow-y-auto max-h-[80vh]">
            <div className="px-6 pt-6 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-black text-slate-900 tracking-tight">Output VAT Receipts — {outputModalMonth.monthName} {year}</h2>
              </div>
              <p className="text-sm text-slate-500 mt-0.5">Sales receipts that make up the output VAT for this month.</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm" style={{ minWidth: '900px' }}>
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    {['Receipt #', 'Invoice Type', 'Date', 'Supplier Name', 'Supplier VAT', 'Account', 'Amount', 'Net of VAT', 'VAT (12%)', 'Document'].map(h => (
                ...
                <tbody className="divide-y divide-slate-100">
                  {outputModalMonth.outputRecords.map(rec => (
                    <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm font-semibold text-slate-800 whitespace-nowrap">{rec.supplierName}</td>
                      <td className="px-4 py-3 font-mono text-xs text-slate-500">{rec.supplierVat || <span className="text-slate-300">—</span>}</td>
                      <td className="px-4 py-3 text-xs text-slate-600">{rec.account}</td>
                      <td className="px-4 py-3 font-mono text-sm font-semibold text-slate-800 text-right whitespace-nowrap">{fmtPHP(rec.amount)}</td>
                      <td className="px-4 py-3 font-mono text-sm text-slate-700 text-right whitespace-nowrap">{fmtPHP(rec.netOfVat)}</td>
                      <td className={`px-4 py-3 font-mono text-sm font-bold text-right whitespace-nowrap ${rec.vat12 < 0 ? 'text-red-600' : 'text-slate-800'}`}>{fmtPHP(rec.vat12)}</td>
                ...
                <tfoot>
                  <tr className="bg-slate-100 border-t-2 border-slate-300">
                    <td colSpan={6} className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-600">Total</td>
                    <td className="px-4 py-3 font-mono text-sm font-black text-slate-900 text-right">
                      {fmtPHP(outputModalMonth.outputRecords.reduce((s, r) => s + r.amount, 0))}
                    </td>
                    <td className="px-4 py-3 font-mono text-sm font-black text-slate-900 text-right">
                      {fmtPHP(outputModalMonth.outputRecords.reduce((s, r) => s + r.netOfVat, 0))}
                    </td>
                    <td className="px-4 py-3 font-mono text-sm font-black text-slate-900 text-right">
                      {fmtPHP(outputModalMonth.outputVAT)}
                    </td>
                    <td />
                  </tr>
                </tfoot>
      ...
      {/* ══════════════════════════════════════════════════════════════
          MODAL 2 — Value-Added Tax Process Modal
      ══════════════════════════════════════════════════════════════ */}
      <Modal
        isOpen={vatModalOpen}
        ...
        size="3xl"
      >
        {selectedQuarter && editComputation && (() => {
          ...
          const qOutputVAT   = selectedQuarter.months.reduce((s, m) => s + m.outputVAT, 0);
          const qInputVAT    = selectedQuarter.months.reduce((s, m) => s + m.inputVAT, 0);

          // Computation derived values
          const totalOutputTax =
            (editComputation.vatableSales * 0.12) +
            (editComputation.salesToGovernment * 0.12);
          const totalAllowableInput = editComputation.inputTaxCarriedOver;
          const totalCurrentPurchasesInput =
            editComputation.capitalGoodsNotExceeding1M +
            editComputation.capitalGoodsExceeding1M +
            editComputation.domesticPurchasesGoods +
            editComputation.importationGoods +
            editComputation.domesticPurchasesServices;
          const totalAvailableInputTax = totalAllowableInput + totalCurrentPurchasesInput;
          const netVatPayable = Math.round((totalOutputTax - totalAvailableInputTax) * 100) / 100;

          return (
            <div className="overflow-y-auto max-h-[85vh]">
              {/* Modal sub-header */}
              <div className="px-6 pt-6 pb-4 border-b border-slate-100">
                <h2 className="text-xl font-black text-slate-900 tracking-tight">Value-Added Tax</h2>
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                    ...
                    <p className="text-lg font-black text-slate-900">{fmtPHP(Math.abs(selectedQuarter.months.reduce((s, m) => s + m.vat12, 0)))}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">Output VAT − Input VAT</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                ...
                <div>
                  ...
                  {/* ATC badge */}
                  <div className="mb-3 inline-flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-1.5">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Alphanumeric Tax Code:</span>
                    <span className="text-xs font-black text-slate-700 font-mono">VB010</span>
                  </div>

                  <div className="rounded-xl border border-slate-200 overflow-hidden">
                    {/* Current Sales and Output Tax */}
                    <div className="px-4 py-2 bg-slate-50 border-b border-slate-200">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Current Sales and Output Tax</p>
                    </div>
                    {[
                      { label: 'Vatable Sales',        key: 'vatableSales' as const },
                      { label: 'Sales to Government',  key: 'salesToGovernment' as const },
                      { label: 'Zero Rated Sales',     key: 'zeroRatedSales' as const },
                      { label: 'Exempt Sales',         key: 'exemptSales' as const },
                    ].map(row => (
                      <div key={row.key} className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        <span className="text-sm text-slate-700">{row.label}</span>
                        <input
                          type="number"
                          min="0"
                          value={editComputation[row.key]}
                          onChange={e => setEditComputation(prev => prev ? { ...prev, [row.key]: parseFloat(e.target.value) || 0 } : prev)}
                          className="w-36 h-7 text-right rounded-lg border border-slate-200 bg-white px-2.5 text-sm font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400"
                        />
                      </div>
                    ))}
                    <div className="flex items-center justify-between px-4 py-2.5 bg-slate-50 border-b border-slate-200">
                      <span className="text-xs font-black uppercase tracking-wide text-slate-600">Total Output Tax (12%)</span>
                      <span className="font-mono text-sm font-black text-slate-800">{fmtNum(totalOutputTax)}</span>
                    </div>

                    {/* Allowable Input Tax */}
                    <div className="px-4 py-2 bg-slate-50 border-b border-slate-200">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Allowable Input Tax</p>
                    </div>
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <span className="text-sm text-slate-700">Input Tax Carried Over from Previous Period</span>
                      <input
                        type="number"
                        min="0"
                        value={editComputation.inputTaxCarriedOver}
                        onChange={e => setEditComputation(prev => prev ? { ...prev, inputTaxCarriedOver: parseFloat(e.target.value) || 0 } : prev)}
                        className="w-36 h-7 text-right rounded-lg border border-slate-200 bg-white px-2.5 text-sm font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400"
                      />
                    </div>
                    <div className="flex items-center justify-between px-4 py-2.5 bg-slate-50 border-b border-slate-200">
                      <span className="text-xs font-black uppercase tracking-wide text-slate-600">Total Allowable Input Tax</span>
                      <span className="font-mono text-sm font-black text-slate-800">{fmtNum(totalAllowableInput)}</span>
                    </div>

                    {/* Current Purchases and Input Tax */}
                    <div className="px-4 py-2 bg-slate-50 border-b border-slate-200">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Current Purchases and Input Tax</p>
                    </div>
                    {[
                      { label: 'Purchase of Capital Goods not exceeding 1M',   key: 'capitalGoodsNotExceeding1M' as const },
                      { label: 'Purchase of Capital Goods exceeding 1M',        key: 'capitalGoodsExceeding1M'    as const },
                      { label: 'Domestic Purchases of Goods (not capital goods)', key: 'domesticPurchasesGoods'   as const },
                      { label: 'Importation of Goods (not capital goods)',       key: 'importationGoods'           as const },
                      { label: 'Domestic Purchase of Services',                  key: 'domesticPurchasesServices'  as const },
                      { label: 'Purchases Not Qualified for Input Tax',           key: 'purchasesNotQualified'      as const },
                    ].map(row => (
                      <div key={row.key} className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        <span className="text-sm text-slate-700">{row.label}</span>
                        <input
                    ...
                    <div className="flex items-center justify-between px-4 py-2.5 bg-slate-50 border-b border-slate-200">
                      <span className="text-xs font-black uppercase tracking-wide text-slate-600">Total Current Purchases Input Tax</span>
                      <span className="font-mono text-sm font-black text-slate-800">{fmtNum(totalCurrentPurchasesInput)}</span>
                    </div>
                    <div className="flex items-center justify-between px-4 py-2.5 bg-slate-100 border-b border-slate-200">
                      <span className="text-xs font-black uppercase tracking-wide text-slate-700">Total Available Input Tax</span>
                      <span className="font-mono text-sm font-black text-slate-900">{fmtNum(totalAvailableInputTax)}</span>
                    </div>

                    {/* NET VAT PAYABLE */}
                    <div className="flex items-center justify-between px-4 py-3.5 bg-slate-800">
                      <span className="text-xs font-black uppercase tracking-widest text-slate-200">NET VAT PAYABLE</span>
                      <span className={`font-mono text-lg font-black ${netVatPayable < 0 ? 'text-red-300' : 'text-white'}`}>
                        {fmtNum(netVatPayable)}
                      </span>
                    </div>
                  </div>

                  {/* Save computation button */}
                  <div className="mt-4 flex justify-end">
                    <button
                      onClick={() => saveComputation(selectedQuarter.id)}
                    ...
                    >
                      <Check size={14} /> Save Computation
                    </button>
                  </div>
                </div>

                {/* Output VAT summary reference */}
                <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Reference — Quarter Totals</p>
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-0.5">Output VAT</p>
                      <p className="font-mono text-sm font-black text-slate-800">{fmtPHP(qOutputVAT)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-0.5">Input VAT</p>
                      <p className="font-mono text-sm font-black text-slate-800">{fmtPHP(qInputVAT)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-0.5">VAT (12%)</p>
                      <p className={`font-mono text-sm font-black ${(qOutputVAT - qInputVAT) < 0 ? 'text-red-600' : 'text-slate-800'}`}>
                        {fmtPHP(qOutputVAT - qInputVAT)}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <Modal isOpen={openCaseOpen} onClose={() => setOpenCaseOpen(false)} title="File Open Case" size="lg">
        <div className="p-6 space-y-4">
          <p className="text-sm text-slate-500">
            Filing a new open case for <strong className="text-slate-900">{client.businessName}</strong> — Value-Added Tax.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select className="w-full h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400">
                <option>BIR — Deficiency VAT</option>
          ...
          <div>
            ...
            <textarea rows={3} placeholder="Describe the case..." className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 resize-none" />
          </div>
