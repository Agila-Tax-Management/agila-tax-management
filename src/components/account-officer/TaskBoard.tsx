...
export function TaskBoard() {
  ...
  const [newAssigneeId, setNewAssigneeId] = useState(AO_TEAM_MEMBERS[0]?.id ?? '');
  ...
  const getAssignee = (assigneeId: string) =>
    AO_TEAM_MEMBERS.find(m => m.id === assigneeId);
  ...
  const resetCreateForm = () => {
    ...
    setNewAssigneeId(AO_TEAM_MEMBERS[0]?.id ?? '');
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button className="bg-[#25238e] hover:bg-[#1a1868] text-white" onClick={() => setIsCreateModalOpen(true)}>
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => { setIsCreateModalOpen(false); resetCreateForm(); }} title="Create New Task" size="lg">
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-10 px-3 bg-white border border-slate-200 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#25238e]"
              >
                {AO_TEAM_MEMBERS.map(m => <option key={m.id} value={m.id}>{m.name} ({m.department})</option>)}
              </select>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input value={newTags} onChange={e => setNewTags(e.target.value)} placeholder="e.g. BIR, Tax Filing, Monthly" />
          </div>
