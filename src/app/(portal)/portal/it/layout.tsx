// src/app/(portal)/portal/it/layout.tsx
'use client';
...
export default function ITPortalLayout({ children }: { children: React.ReactNode }) {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const router = useRouter();
  const pathname = usePathname();

  const moduleRoot = '/portal/it';
  const isRoot = pathname === moduleRoot;

  useEffect(() => {
    document.title = 'IT Portal | Agila Tax Management System';
  }, []);

  return (
    <RoleProvider>
      <div className="flex h-screen overflow-hidden bg-slate-50">
        <ITSidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />
