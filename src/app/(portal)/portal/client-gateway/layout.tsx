// src/app/(portal)/portal/client-gateway/layout.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { useRouter, usePathname } from 'next/navigation';
import { ClientGatewaySidebar } from '@/components/client-gateway/ClientGatewaySidebar';
...
export default function ClientGatewayLayout({ children }: { children: React.ReactNode }) {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const router = useRouter();
  const pathname = usePathname();

  const moduleRoot = '/portal/client-gateway';
  const isRoot = pathname === moduleRoot;

  useEffect(() => {
    document.title = 'Client Gateway | Agila Tax Management System';
  }, []);

  return (
    <RoleProvider>
      <div className="flex h-screen overflow-hidden bg-slate-50">
        <ClientGatewaySidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />
