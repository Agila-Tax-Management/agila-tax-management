// src/app/(portal)/portal/sales/services/one-time/new-service/components/NewOneTimeServiceForm.tsx
'use client';
...
import { ArrowLeft, FileText, Save, Search, Plus, X, Layout } from 'lucide-react';
              ...
              {filtered.length > 0 ? (
                filtered.map((item) => (
                  <button
                    key={item.id}
                    type="button"
                    className="w-full text-left px-3 py-2.5 text-xs hover:bg-blue-50 text-slate-800 transition-colors flex items-center gap-2"
...
export function NewOneTimeServiceForm(): React.ReactNode {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        error('Failed to create service', data.error ?? 'An error occurred.');
        return;
      }
      success('Service created', `"${data.data.name}" was saved successfully.`);
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 tracking-tight">New One-Time Service</h2>
            <p className="text-xs text-slate-500">Create a one-time service offering</p>
          </div>
        </div>
      </div>
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              <div className="p-2 border-b border-slate-100 bg-slate-50">
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" size={13} />
                  <input
                    className="w-full pl-8 pr-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 opacity-60"
                ...
                <div className="flex flex-wrap gap-1.5">
                  {form.cityIds.map((id) => {
                    ...
                    return item ? (
                      <span
                        key={id}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800"
                      >
                        {item.name}
